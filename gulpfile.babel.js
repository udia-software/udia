...
const paths = {
  client: {
    ...
    scripts: [
      `${clientPath}/**/!(*.spec|*.mock|semantic.js).js`
    ],
...
gulp.task('copy:semantic:dev', () => {
  return gulp.src('semantic/dist/*')
    .pipe(flatten())
    .pipe(gulp.dest(`${clientPath}/assets/semantic`))
});
gulp.task('copy:semantic:dist', () => {
  return gulp.src('semantic/dist/*')
    .pipe(flatten())
    .pipe(gulp.dest(`${paths.dist}/${clientPath}/assets/semantic`));
});
