defmodule UdiaWeb.CommentControllerTest do
  ...
  describe "index" do
    ...
    test "lists a user's comments on index", %{conn: conn} do
      user = insert_user(@user_params)
      post = insert_post(user, @post_params)
      comment = insert_comment(user, post, @create_attrs)

      another_user = insert_user()
      insert_comment(another_user, post, @create_attrs)

      # Test only getting the user's comments
      conn = build_conn()
      |> get(comment_path(conn, :index), %{"username": user.username})
